...
###########################################################################################################################################
# NEW CODE BLOCK - Single crosstab with p-values, margin totals, and n counts
###########################################################################################################################################

# Generate total crosstabs report
def crosstabs(col_left, col_top, df):
    # Create cross tab
    ...
    # Convert to single digit percentages
    ...
    # Drop last row because we only want row counts
    crosstab_totals = crosstab_totals[:-1]
    ...
    # Have total add to 100
    crosstab['Total'] = np.where(
        (crosstab['Total'] < 100) & (crosstab['Total'] > 98),
        100,
        crosstab['Total']
    )
    
    # Apply crosstab esthetics
    ...
    # Convert to string to have data visually appear as int
    ...
    # Display col_top feature name
    crosstab.columns = pd.MultiIndex.from_tuples(col_names_list)
    crosstab['.'] = ''
    
    return crosstab

# * **top_col** = Top banner variable
# * **left_col** = Variable under review
# * **df** =  Data frame


###########################################################################################################################################
# NEW CODE BLOCK - Crosstabs report with all combinations
###########################################################################################################################################

def crosstab_report(df, drop_cols):
    # Drop column
    freq_book_df = df.drop(
        drop_cols,
        axis = 1,
        errors = 'ignore'
    )
    
    # Loop through creating all crosstabs
    final_crosstab_list = []
